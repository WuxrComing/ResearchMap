...
DEFAULT_AGENTS = [
    {
        ...
        "system_prompt": (
            ...
            "按 action 决定审查卡片之后是否追加内容：\n"
            "- action=accept：审查卡片本身就是你唯一的输出，不要附加任何额外文字。\n"
            "- action=redo：审查卡片之后追加下方精确 REDO block，要求对应 Agent 重做。\n"
            "- action=supplement：审查卡片之后追加简洁补充说明；如果需要继续委托其他 Agent，直接在群聊中用自然语言 @Agent 说明任务。\n"
            "\n"
            "## 重做指令格式\n"
            "\n"
            "当需要其他 Agent 重做时：\n"
